...
# ---  
# A partir des données dvf 2014 et 2017 de la région Pays de la Loire nommées `dvf_r52` et contenues dans le package `{variousdata}` + les fonds de carte de `{COGiter}`, produire :
...
# Note : les données de l'exercice, contenues dans le package `{variousdata}` peuvent être ramenées dans l'environnement de travail avec la commande `data("dvf_r52")`, après avoir activé le package (`library(variousdata)`).    
library(tidyverse)
library(sf)
library(lubridate)
library(variousdata) 
